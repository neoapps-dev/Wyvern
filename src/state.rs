...
pub struct WyvernState<BackendData: Backend + 'static> {
    ...
    pub handle: LoopHandle<'static, WyvernState<BackendData>>,
    ...
    pub seat_state: SeatState<WyvernState<BackendData>>,
...
impl<BackendData: Backend> ClientDndGrabHandler for WyvernState<BackendData> {
    fn started(
        &mut self,
...
impl<BackendData: Backend> ServerDndGrabHandler for WyvernState<BackendData> {
    fn send(&mut self, _mime_type: String, _fd: OwnedFd, _seat: Seat<Self>) {
        unreachable!("Wyvern doesn't do server-side grabs");
    }
}
delegate_data_device!(@<BackendData: Backend + 'static> WyvernState<BackendData>);

impl<BackendData: Backend> OutputHandler for WyvernState<BackendData> {}
delegate_output!(@<BackendData: Backend + 'static> WyvernState<BackendData>);

impl<BackendData: Backend> SelectionHandler for WyvernState<BackendData> {
...
impl<BackendData: Backend> PrimarySelectionHandler for WyvernState<BackendData> {
    fn primary_selection_state(&self) -> &PrimarySelectionState {
        &self.primary_selection_state
    }
}
delegate_primary_selection!(@<BackendData: Backend + 'static> WyvernState<BackendData>);

impl<BackendData: Backend> DataControlHandler for WyvernState<BackendData> {
    fn data_control_state(&self) -> &DataControlState {
        &self.data_control_state
    }
}

delegate_data_control!(@<BackendData: Backend + 'static> WyvernState<BackendData>);

impl<BackendData: Backend> ShmHandler for WyvernState<BackendData> {
    fn shm_state(&self) -> &ShmState {
        &self.shm_state
    }
}
delegate_shm!(@<BackendData: Backend + 'static> WyvernState<BackendData>);

impl<BackendData: Backend> SeatHandler for WyvernState<BackendData> {
    type KeyboardFocus = KeyboardFocusTarget;
    type PointerFocus = PointerFocusTarget;
    type TouchFocus = PointerFocusTarget;

    fn seat_state(&mut self) -> &mut SeatState<WyvernState<BackendData>> {
        &mut self.seat_state
    }
...
delegate_seat!(@<BackendData: Backend + 'static> WyvernState<BackendData>);

impl<BackendData: Backend> TabletSeatHandler for WyvernState<BackendData> {
    fn tablet_tool_image(&mut self, _tool: &TabletToolDescriptor, image: CursorImageStatus) {
        // TODO: tablet tools should have their own cursors
        self.cursor_status = image;
    }
}
delegate_tablet_manager!(@<BackendData: Backend + 'static> WyvernState<BackendData>);

delegate_text_input_manager!(@<BackendData: Backend + 'static> WyvernState<BackendData>);

impl<BackendData: Backend> InputMethodHandler for WyvernState<BackendData> {
...
delegate_input_method_manager!(@<BackendData: Backend + 'static> WyvernState<BackendData>);

impl<BackendData: Backend> KeyboardShortcutsInhibitHandler for WyvernState<BackendData> {
...
delegate_keyboard_shortcuts_inhibit!(@<BackendData: Backend + 'static> WyvernState<BackendData>);

delegate_virtual_keyboard_manager!(@<BackendData: Backend + 'static> WyvernState<BackendData>);

delegate_pointer_gestures!(@<BackendData: Backend + 'static> WyvernState<BackendData>);

delegate_relative_pointer!(@<BackendData: Backend + 'static> WyvernState<BackendData>);

impl<BackendData: Backend> PointerConstraintsHandler for WyvernState<BackendData> {
...
delegate_pointer_constraints!(@<BackendData: Backend + 'static> WyvernState<BackendData>);

delegate_viewporter!(@<BackendData: Backend + 'static> WyvernState<BackendData>);

impl<BackendData: Backend> XdgActivationHandler for WyvernState<BackendData> {
...
delegate_xdg_activation!(@<BackendData: Backend + 'static> WyvernState<BackendData>);

impl<BackendData: Backend> XdgDecorationHandler for WyvernState<BackendData> {
...
delegate_xdg_decoration!(@<BackendData: Backend + 'static> WyvernState<BackendData>);

delegate_xdg_shell!(@<BackendData: Backend + 'static> WyvernState<BackendData>);
delegate_layer_shell!(@<BackendData: Backend + 'static> WyvernState<BackendData>);
delegate_presentation!(@<BackendData: Backend + 'static> WyvernState<BackendData>);

impl<BackendData: Backend> FractionalScaleHandler for WyvernState<BackendData> {
    fn new_fractional_scale(
        ...
        // of the space (which in case of Wyvern will also be the output a toplevel will
...
delegate_fractional_scale!(@<BackendData: Backend + 'static> WyvernState<BackendData>);

impl<BackendData: Backend + 'static> SecurityContextHandler for WyvernState<BackendData> {
    fn context_created(
        &mut self,
...
delegate_security_context!(@<BackendData: Backend + 'static> WyvernState<BackendData>);

#[cfg(feature = "xwayland")]
impl<BackendData: Backend + 'static> XWaylandKeyboardGrabHandler for WyvernState<BackendData> {
...
#[cfg(feature = "xwayland")]
delegate_xwayland_keyboard_grab!(@<BackendData: Backend + 'static> WyvernState<BackendData>);

#[cfg(feature = "xwayland")]
delegate_xwayland_shell!(@<BackendData: Backend + 'static> WyvernState<BackendData>);

impl<BackendData: Backend> XdgForeignHandler for WyvernState<BackendData> {
    fn xdg_foreign_state(&mut self) -> &mut XdgForeignState {
        &mut self.xdg_foreign_state
    }
}
smithay::delegate_xdg_foreign!(@<BackendData: Backend + 'static> WyvernState<BackendData>);

smithay::delegate_single_pixel_buffer!(@<BackendData: Backend + 'static> WyvernState<BackendData>);

smithay::delegate_fifo!(@<BackendData: Backend + 'static> WyvernState<BackendData>);
        ...
        let ret = self
            ...
            .insert_source(xwayland, move |event, _, data| match event {
                XWaylandEvent::Ready {
                    ...
                    let xwayland_scale = std::env::var("WYVERN_XWAYLAND_SCALE")
                        .ok()
