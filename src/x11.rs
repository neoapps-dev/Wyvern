...
use crate::{
    drawing::*,
    render::*,
    state::{take_presentation_feedback, Backend, WyvernState},
};
#[cfg(feature = "egl")]
...
impl DmabufHandler for WyvernState<X11Data> {
    ...
    fn dmabuf_imported(
        ...
        if self
            ...
            .is_ok()
        {
            let _ = notifier.successful::<WyvernState<X11Data>>();
        } else {
            notifier.failed();
        }
    }
}
delegate_dmabuf!(WyvernState<X11Data>);
...
pub fn run_x11() {
    ...
    let skip_vulkan = std::env::var("WYVERN_NO_VULKAN")
        .map(|x| {
            x == "1"
                || x.to_lowercase() == "true"
                || x.to_lowercase() == "yes"
    ...
    let dmabuf_global = dmabuf_state.create_global_with_default_feedback::<WyvernState<X11Data>>(
        &display.handle(),
    ...
    let _global = output.create_global::<WyvernState<X11Data>>(&display.handle());
