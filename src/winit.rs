...
impl DmabufHandler for WyvernState<WinitData> {
    ...
    fn dmabuf_imported(
        ...
        if self
            ...
            .is_ok()
        {
            let _ = notifier.successful::<WyvernState<WinitData>>();
        } else {
            notifier.failed();
        }
    }
}
delegate_dmabuf!(WyvernState<WinitData>);
...
pub fn run_winit() {
    ...
    let _global = output.create_global::<WyvernState<WinitData>>(&display.handle());
    ...
    let dmabuf_state = if let Some(default_feedback) = dmabuf_default_feedback {
        ...
        let dmabuf_global = dmabuf_state
            .create_global_with_default_feedback::<WyvernState<WinitData>>(
                &display.handle(),
                &default_feedback,
            );
        ...
        let mut dmabuf_state = DmabufState::new();
        let dmabuf_global =
            dmabuf_state.create_global::<WyvernState<WinitData>>(&display.handle(), dmabuf_formats);
