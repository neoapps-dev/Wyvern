...
impl<BackendData: Backend> XdgShellHandler for WyvernState<BackendData> {
    ...
    fn move_request(&mut self, surface: ToplevelSurface, seat: wl_seat::WlSeat, serial: Serial) {
        let seat: Seat<WyvernState<BackendData>> = Seat::from_resource(&seat).unwrap();
        ...
        if let Some(touch) = seat.get_touch() {
            if touch.has_grab(serial) {
                    ...
                    .contains(xdg_toplevel::State::Maximized)
                {
                    ...
                    //
                    // but for Wyvern needs setting location to pointer location is fine
