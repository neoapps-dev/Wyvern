...
use crate::{
    ...
    render::*,
    shell::WindowElement,
    state::{take_presentation_feedback, update_primary_scanout_output, Backend, WyvernState},
};
use crate::{
...
impl DmabufHandler for WyvernState<UdevData> {
    ...
    fn dmabuf_imported(
            ...
            .is_ok()
        {
            ...
            let _ = notifier.successful::<WyvernState<UdevData>>();
        } else {
            notifier.failed();
        }
    }
}
delegate_dmabuf!(WyvernState<UdevData>);
...
pub fn run_udev() {
     ...
     */
    let primary_gpu = if let Ok(var) = std::env::var("WYVERN_DRM_DEVICE") {
        ...
        .insert_source(notifier, move |event, &mut (), data| match event {
            ...
            SessionEvent::ActivateSession => {
                    ...
                    .map(|(handle, backend)| (*handle, backend))
                {
                    ...
                    if let Some(lease_global) = backend.leasing_global.as_mut() {
                        lease_global.resume::<WyvernState<UdevData>>();
                    }
                    data.handle
    ...
    let global = dmabuf_state.create_global_with_default_feedback::<WyvernState<UdevData>>(
        &display_handle,
        &default_feedback,
    );
        ...
        .and_then(|x| x.ok())
    {
        if let Some(backend) = state.backend_data.backends.get(&primary_node) {
            ...
            if supports_syncobj_eventfd(&import_device) {
                let syncobj_state =
                    DrmSyncobjState::new::<WyvernState<UdevData>>(&display_handle, import_device);
...
impl DrmLeaseHandler for WyvernState<UdevData> {
...
impl Drop for SurfaceData {
    fn drop(&mut self) {
        if let Some(global) = self.global.take() {
            self.dh.remove_global::<WyvernState<UdevData>>(global);
        }
    }
}
...
impl WyvernState<UdevData> {
    fn device_added(&mut self, node: DrmNode, path: &Path) -> Result<(), DeviceAddError> {
        ...
        let registration_token = self
            ...
            .insert_source(
                notifier,
                move |event, metadata, data: &mut WyvernState<_>| match event {
        ...
        self.backend_data.backends.insert(
            ...
            BackendData {
                ...
                leasing_global: DrmLeaseState::new::<WyvernState<UdevData>>(
                    &self.display_handle,
                    &node,
                )
                .inspect_err(|err| {
    ...
    fn connector_connected(
        ...
        if non_desktop {
            ...
            if let Some(lease_state) = device.leasing_global.as_mut() {
                lease_state.add_connector::<WyvernState<UdevData>>(
                    connector.handle(),
                    output_name,
                    format!("{} {}", make, model),
                );
            ...
            let global = output.create_global::<WyvernState<UdevData>>(&self.display_handle);
            ...
            let disable_direct_scanout = std::env::var("WYVERN_DISABLE_DIRECT_SCANOUT").is_ok();
    ...
    fn device_removed(&mut self, node: DrmNode) {
        ...
        if let Some(mut backend_data) = self.backend_data.backends.remove(&node) {
            if let Some(mut leasing_global) = backend_data.leasing_global.take() {
                leasing_global.disable_global::<WyvernState<UdevData>>();
            }
    ...
    fn render_surface(&mut self, node: DrmNode, crtc: crtc::Handle, frame_target: Time<Monotonic>) {
        ...
        let frame = self
            .backend_data
            .pointer_image
            .get_image(cfg.display.scale /*scale*/, self.clock.now().into());
